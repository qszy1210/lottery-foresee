...
- 在主推荐之后追加 2 注扰动号码：
  - 「💥 震荡推荐」：基于主推荐做扰动重组，引入冷门号
  - 「🎲 无规律震荡」：基于主推荐 #1 纯随机替换 2 个号码，不依赖任何概率权重
- 记录每次生成结果（历史生成记录）
...
- `POST /ssq/predict`
  ...
  - 返回：`[{reds:[...6], blue:int, score:float, kind:"main"|"shock"|"random_shock"}, ...]`
  - 默认在 `recommend_count` 组主推荐之后追加 2 注：1 个 `kind="shock"`（震荡）+ 1 个 `kind="random_shock"`（无规律震荡），共 `N+2` 项
...
## 震荡推荐（Shock & Random Shock）

主推荐基于概率收敛，会反复命中"热门号码"。为了在不放弃概率信号的前提下留出"反向探索"的余地，每次预测会**额外生成 2 注扰动号码**，附加在主推荐列表末尾。
...
### 🎲 无规律震荡（`kind="random_shock"`）

只关心**分数最高的主推荐 #1**，从中**纯随机**替换 2 个号码：

1. 在主推荐 #1 的红球/前区里随机挑 2 个位置；
2. 从该注**没出现过**的号码全集中**均匀随机**抽 2 个填入；
3. 蓝球/后区**保持不变**；
4. 整个过程**不使用任何概率/频率/反向权重**，体现"无规律"含义。

### 共同约束

- 两类扰动注都会被打分并写入历史记录，参与后续比对统计；
- 主推荐的随机种子 (`seed`) 不影响扰动注的 RNG，三者互不污染；
- 服务层支持 `include_shock=False` / `include_random_shock=False` 单独关闭（默认都开启），`random_shock_perturb` 可调整替换个数；
- 飞书卡片与前端 UI 用 **紫色 💥** / **橙色 🎲** 两种徽章分别标识。
